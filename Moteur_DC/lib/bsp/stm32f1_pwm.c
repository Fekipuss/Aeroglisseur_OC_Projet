...
void PWM_toto(void)
{
	PWM_run(TIMER3_ID, TIM_CHANNEL_1, FALSE,1000, 10, FALSE);
}


void PWM_test(void)
{
	PWM_run(TIMER4_ID, TIM_CHANNEL_3, FALSE, 1, 50, FALSE);
	//PWM_run(TIMER4_ID, TIM_CHANNEL_4, 1, 50, FALSE);
	//PWM_run(TIMER3_ID, TIM_CHANNEL_1, 1000000, 99, FALSE);
	PWM_run(TIMER3_ID, TIM_CHANNEL_1, FALSE,1000, 50, FALSE);
	PWM_run(TIMER1_ID, TIM_CHANNEL_2, FALSE,1000, 1, FALSE);
	PWM_run(TIMER1_ID, TIM_CHANNEL_1, TRUE,1000, 10, FALSE);
	PWM_run(TIMER1_ID, TIM_CHANNEL_2, TRUE,1000, 10, FALSE);
	PWM_run(TIMER1_ID, TIM_CHANNEL_3, TRUE,1000, 25, FALSE);
	//PWM_run(TIMER3_ID, TIM_CHANNEL_3, 100, 90, FALSE);
	//PWM_run(TIMER3_ID, TIM_CHANNEL_4, 100, 99, FALSE);
}
...
void PWM_run(timer_id_e timer_id, uint16_t TIM_CHANNEL_x, bool_e negative_channel, uint32_t period_us, uint32_t duty, bool_e remap)
{
	...
	if(handler != NULL)
	{
		...
		PWM_set_duty(timer_id, TIM_CHANNEL_x, negative_channel, duty);
	}
}

/** @brief	fonction qui règle le rapport cyclique sur le canal demandé.
 * @func	PWM_set_duty(timer_id_e timer_id, uint16_t TIM_CHANNEL_x, bool_e negative_channel, uint16_t duty)
 * @param	timer_id : TIMERX_ID, où X est le numéro du timer
 * @param	TIM_CHANNEL_x, où x est le numéro du canal
 * @param	negative channel : TRUE s'il s'agit de configuer l'un des canaux CH1_N, CH2_N, CH3_N. False sinon.
 ...
 */
void PWM_set_duty(timer_id_e timer_id, uint16_t TIM_CHANNEL_x, bool_e negative_channel, uint32_t duty)
{
	TIM_HandleTypeDef * handler = NULL;
	handler = TIMER_get_phandler(timer_id);
	duty = MIN(100,duty);	//Ecretage... Le rapport cyclique ne peut donc pas être plus grand que 100 !
	duty = ((duty*handler->Init.Period)/100);
